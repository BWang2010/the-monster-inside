...
final_background = pygame.image.load("final.png").convert()

man_x = 100
man_y = HEIGHT - 150
monster_x = WIDTH // 2 - 100
...
def display_end_sequence():
    global fade_alpha
    fade_text("You lose. Reflect on your actions.")
    fade_text("Your past actions will change your future.")
    fade_text("You are your own enemy.")
    fade_alpha = 255
    while fade_alpha > 0:
        fade_surface.set_alpha(fade_alpha)
...
while running:
    ...
    if player_is_monster:
        if start_hurling_insults and insult_index < len(insults[1]):
            ...
            if insult_timer > 2.5:  # Display an insult every 2.5 seconds
                ...
                insult_index += 1
                insult_timer = 0
                if insult_index == len(insults[1]):  # When all insults are displayed
                    time.sleep(1)
                    fade_out_in()
                    if stage == 6:  # End the game after repeating Levels 1-3 as the monster
                        display_end_sequence()
                        running = False
    else:
        if man_direction == "left":
            screen.blit(man_left[man_frame], (man_x, man_y-300))
        elif man_direction == "right":
            screen.blit(man_right[man_frame], (man_x, man_y-300))
        else:
            screen.blit(man_right[0], (man_x, man_y-300))
    ...
    if all_keys_pressed() and stage <= 3:  # Handle levels as the man
        fade_out_in()
        if stage < 3:
            stage += 1
            reset_stage()
        elif stage == 3:  # Transition to being the monster
            stage = 4
            reset_stage()
            player_is_monster = True
            start_hurling_insults = True

    if all_keys_pressed() and stage >= 4:  # Handle levels as the monster
        fade_out_in()
        if stage < 6:  # Repeat Levels 1-3 but as the monster
            stage += 1
            reset_stage()
        elif stage == 6:  # End the game after all insults
            display_end_sequence()
            running = False
