...
def display_end_screen():
    ...
    y_offset = 50
    for insult in all_insults:
        text_surface = font.render(insult, True, WHITE)
        screen.blit(text_surface, (WIDTH // 2 - text_surface.get_width() // 2, y_offset))
        y_offset += 40
    final_message = "You were your enemy all along."
    final_surface = font.render(final_message, True, WHITE)
    screen.blit(final_surface, (WIDTH // 2 - final_surface.get_width() // 2, y_offset + 40))
    pygame.display.flip()
    time.sleep(5)
...
while running:
    ...
    if player_is_monster:
        if start_hurling_insults and insult_index < len(insults[1]):
            ...
            if insult_timer > 2:  # Display an insult every 2 seconds
