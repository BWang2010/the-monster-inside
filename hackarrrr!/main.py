...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if stage == 1 and not player_is_monster:
    ...
    if player_is_monster and stage == 2:
        if current_insult < len(insults[stage]):
            text = font.render(insults[stage][current_insult], True, WHITE)
    ...
    if len(messages) == 3 and stage == 1:
        fading = True

    if current_insult == len(insults[stage]) and stage == 2:
        fading = True
